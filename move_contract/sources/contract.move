module contract::ContractDatabase {
    ...
    // new function for multi sign
    public entry fun transfer_two_by_two(
        first: &signer,
        second: &signer,
        amount_first: u64,
        amount_second: u64,
    ...
    // new function for multi sign (20240902)
    #[event]
    ...
    entry public fun multi_signer(
        first_signer: &signer,
        nexr_signer: &signer
    ) {
        event::emit(
            Event{
                addr_1: signer::address_of(first_signer),
                addr_2: signer::address_of(nexr_signer)
            }
        );
    }
